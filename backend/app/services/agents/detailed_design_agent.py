...
class DetailedDesignAgent:
    ...
    async def run(
        ...
        try:
            logger.info("stage=detailed_design event=llm_call_start")
            result = await execute_agent(self.agent, prompt)
            logger.info("stage=detailed_design event=llm_call_complete")
            typed = parse_model_result("detailed_design", result, DesignResult)
            logger.info("stage=detailed_design event=validation_success")
